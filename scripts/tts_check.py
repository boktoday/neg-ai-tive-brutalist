"""Check TTS task status and download result."""
...
for attempt in range(20):
    ...
    s = data.get("base_resp", {}).get("status_msg", "")
    if s == "success":
        audio_url = data.get("data", {}).get("audio_url", "")
        if audio_url:
            print(f"Got audio URL, downloading...")
            audio = requests.get(audio_url, timeout=60)
            out_path = os.path.join(out_dir, "test-intro.mp3")
            with open(out_path, "wb") as f:
                f.write(audio.content)
            print(f"Saved: {len(audio.content)//1024}KB")
        break
