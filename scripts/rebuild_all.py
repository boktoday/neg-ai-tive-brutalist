"""Delete old Printful products and recreate with correct Bevan PNGs."""
...
for p in prods:
    if p["synced"] > 0 or p["synced"] == 0:
        curl("DELETE", f"/sync/product/{p['id']}")
        print(f"  DEL: {p['name'][:45]}")
    time.sleep(0.3)

time.sleep(3)

# Create all 32 products with a 2-second delay between each
print(f"\nCreating {len(products)} products with Bevan font designs...")
results = {}
for name, design, vids, prices in products:
    ...
    pid = r.get("result", {}).get("id")
    results[name] = pid
    print(f"  {'OK' if pid else 'FAIL'}: {name}")
    time.sleep(2.2)

print(f"\n=== {len(results)} products in store ===")
