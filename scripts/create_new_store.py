"""Create all rebranded products in the NEW neg-AI-tive Printful store."""
...
products = [
    # BW = black text on white (for white/light shirts), WB = white text on black (for black/dark shirts)
    ...
    ("ACTUAL INTELLIGENCE - Premium T-Shirt (Black)", "actual-intel-tshirt-wb.png", tshirt_v, ["34.99"]*4+["36.99"]),
    ("ACTUAL INTELLIGENCE - Premium T-Shirt (White)", "actual-intel-tshirt-bw.png", tshirt_v, ["34.99"]*4+["36.99"]),
...
def create(name, design, vids, prices):
    url = f"{PNG}/{design}"
    payload = {
    ...
    cmd = ["curl", "-s", "-X", "POST", "-H", f"Authorization: Bearer {NEW_TOKEN}", "-H", "Content-Type: application/json", "-d", json.dumps(payload), f"{BASE}/store/products"]
...
print(f"Creating {len(products)} products in new store...\n")
for name, design, vids, prices in products:
    create(name, design, vids, prices)
    time.sleep(0.8)

print("\nDone! Check https://www.printful.com/dashboard/sync?store=18264867")
