"""Continue creating remaining Printful products."""
...
r = subprocess.run(["curl", "-s", "-H", f"Authorization: Bearer {TOKEN}", f"{BASE}/sync/products?limit=30"], capture_output=True, text=True, timeout=15)
d = json.loads(r.stdout)
existing = set()
for p in d.get("result", []):
    if p.get("synced") == 5:
        existing.add(p["name"].strip())

print(f"Already have {len(existing)} active products")
print(f"Need to create {len(all_products) - len(existing)} more\n")
...
for name, design, vids, prices in all_products:
    ...
    try:
        ...
        pid = d2.get("result", {}).get("id")
        if pid:
            count += 1
            print(f"  OK: {name} (ID: {pid})")
        else:
            print(f"  FAIL: {name} — {d2.get('error',{}).get('message','')}")
    except:
        print(f"  FAIL: {name} — rate limited")
        time.sleep(65)
        continue
    time.sleep(2.5)

print(f"\nCreated {count} more products!")
