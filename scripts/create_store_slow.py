"""Create products with proper rate limiting (1 per 2 seconds)."""
...
print(f"Creating {len(products)} products (1 every 2 seconds)...")
results = {}
for name, design, vids, prices in products:
    ...
    results[name] = pid
    status = "OK" if pid else f"FAIL"
    print(f"  {status}: {name} (ID: {pid})")
    time.sleep(2.2)

print(f"\n=== Product IDs for website links ===")
